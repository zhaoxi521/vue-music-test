  ...
  export default{
    ...
    methods: {
      ...
      onShortcutTouchStart(e) {
        const targetIndex = getData(e.target, 'index')
      ...
      onShortcutTouchMove(e) {
        ...
        const targetIndex = parseInt(this.touch.targetIndex) + delta
        this._scrollTo(targetIndex)
      },
      scroll(pos) {
        this.scrollY = pos.y
      },
      _scrollTo(targetIndex) {
        if (!targetIndex && targetIndex !== 0) return
        if (targetIndex < 0) {
          targetIndex = 0
        } else if (targetIndex > this.listHeight.length - 2) {
          targetIndex = this.listHeight.length - 2
        }
        this.scrollY = -this.listHeight[parseInt(targetIndex)]
    ...
    watch: {
      ...
      diff(newVal) {
        let fixTop = (newVal > 0 && newVal < DIFF_NUM) ? (newVal - DIFF_NUM) : 0
